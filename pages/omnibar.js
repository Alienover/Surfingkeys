  ...
  self.createURLItem = function(b, rxp) {
    ...
    if (!type) {
      if (b.hasOwnProperty('lastVisitTime')) {
        ...
        additional += `<span class=omnibar_visitcount> (${
          b.visitCount
        })</span>`;
        uid = 'H' + b.url;
      } else if (b.hasOwnProperty('dateAdded')) {
        type = '⭐';
        additional = `<span class=omnibar_folder>@ ${bookmarkFolders[b.parentId]
          .title ||
    ...
    var li = createElement(
      `<li><div class="title">${type} ${self.highlight(
        rxp,
        htmlEncode(b.title)
  ...
  function _listResultPage() {
    ...
    self.listResults(_page, function(b) {
      ...
      if (b.hasOwnProperty('html')) {
        ...
        li = createElement(
          `<li><div class="title">▷ ${self.highlight(rxp, b.title)}</div></li>`
        );
    ...
    if (fi) {
      ...
      if (url.indexOf(':') === -1) {
        try {
          ...
          url =
            SearchEngine.aliases[runtime.conf.defaultSearchEngine].url + url;
        }
      }
    }
...
var Commands = (function() {
  ...
  self.onInput = function() {
    ...
    if (candidates.length) {
      Omnibar.listResults(candidates, function(c) {
        var li = createElement(
          `<li>${c}<span class=annotation>${htmlEncode(
...
var OmniQuery = (function() {
  ...
  self.onTabKey = function() {
    Omnibar.input.value = Omnibar.resultsDiv.querySelector(
      'li.focused'
    ).innerText;
  };
