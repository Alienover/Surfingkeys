...
Front.renderHeaderDescription = function() {
  var words = Normal.mappings
    ...
    .map(function(w) {
      ...
      if (
        meta.annotation &&
        meta.annotation.length &&
        meta.feature_group === 99
      ) {
        return `<div><span class=kbd-span><kbd>${htmlEncode(
          w
        )}</kbd></span><span class=annotation>${meta.annotation}</span></div>`;
      }
      return null;
    })
    .filter(function(w) {
  ...
  content.style.height = window.innerHeight - desc.offsetHeight - 20 + 'px';
};
