...
// ********************* CUSTOM KEYMAPS **********************
// Use the vim-style key to scroll half page
//
map('<Ctrl-d>', 'd');
map('<Ctrl-u>', 'e');
unmap('d');
...
// Bind keys to move next or previous tab
map('<Ctrl-h>', 'E');
map('<Ctrl-l>', 'R');
unmap('E');
unmap('R');
  ...
  runtime.command(
    ...
    function(response) {
      if (response.tabError && response.tabError.length) {
        ...
        Front.showPopup(
          "<table style='width:100%'>{0}</table>".format(errors.join(''))
        );
      } else {
...
mapkey('I', '#1Go to edit box with vim editor', function() {
  Hints.create('input, textarea, *[contenteditable=true], select', function(
    element
  ) {
    Front.showEditor(element);
  });
