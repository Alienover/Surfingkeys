var Mode = (function() {
      ...
      this.addEventListener = function(evt, handler) {
        ...
        if (_listenedEvents.indexOf(evt) === -1) {
          ...
          window.addEventListener(
            evt,
            handleStack.bind(handleStack, evt),
            true
          );
        }
  ...
  function onAfterHandler(mode, event) {
    if (event.sk_stopPropagation) {
      ...
      mode.stopKeyupPropagation =
        event.type === 'keydown' && mode.enableKeyupMerging ? event.keyCode : 0;
    }
  }
...
var Normal = (function() {
      ...
      runtime.command(
        {
          ...
          blacklistPattern: runtime.conf.blacklistPattern
            ? runtime.conf.blacklistPattern.toJSON()
            : '',
        },
        function(resp) {
          if (resp.disabled) {
      ...
      elm.skScrollBy = function(x, y) {
        if (runtime.conf.smartPageBoundary) {
          if (document.scrollingElement.scrollTop === 0 && y <= 0) {
            previousPage() &&
              Front.showBanner('Top margin hit, jump to previous page');
          } else if (
  ...
  function getScrollableElements() {
    var nodes = listElements(document.body, NodeFilter.SHOW_ELEMENT, function(
      n
    ) {
      return (
  ...
  function changeScrollTarget(silent) {
    ...
    if (scrollNodes.length > 0) {
      Hints.create(scrollNodes, function(element) {
        scrollIndex = scrollNodes.indexOf(element);
  ...
  self.mappings.add('f', {
    annotation:
      'Open a link, press SHIFT to flip hints if they are overlapped.',
    feature_group: 1,
    repeatIgnore: true,
    code: function() {
